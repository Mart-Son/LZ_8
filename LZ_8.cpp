...
int main()
{
	...
	printf("\nПорядок обхода графа в ширину ото всех вершин (возможны повторяющиеся вершины в связи с обновлением расстояний): ");
	for(i=0;i<S;i++)
		{
			printf("%\n%3d| ", i+1);
			BFS(i, G, Distance[i], S);
		}
	printf("\nМатрица расстояний от i-й вершины до всех остальных: ");
	for(i=0;i<S;i++)
		{
			printf("\n%3d|", i+1);
			for(j=0;j<S;j++)
